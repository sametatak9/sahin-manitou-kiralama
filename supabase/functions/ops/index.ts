...
import { providerAvailability } from '../_shared/ai/index.ts';
...
async function status(db: Db) {
  ...
  return { ai: providerAvailability(), connectors, worker_last_seen: lastRun?.created_at ?? null, canva_connected: Boolean(canva?.length), redirect_uri: REDIRECT_URI() };
}

// ── OAuth ───────────────────────────────────────────────────────────────────
async function oauthStart(db: Db, userId: string, provider: string) {
  const def = connectorByKey(provider === 'meta' ? 'instagram' : provider);
