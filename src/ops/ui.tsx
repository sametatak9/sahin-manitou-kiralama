...
const BTN: Record<BtnVariant, string> = {
  primary: 'bg-brand-green text-ink-950 hover:bg-emerald-400 shadow-[0_0_0_1px_rgba(61,170,92,.4),0_8px_24px_-8px_rgba(61,170,92,.6)]',
  ...
  warn: 'bg-amber-400 text-ink-950 hover:bg-amber-300',
};
...
const STATE_META: Record<StateKind, { icon: typeof Bot; title: string; tone: string }> = {
  ...
  error: { icon: AlertTriangle, title: 'Bir hata oluştu', tone: 'text-rose-300' },
  permission: { icon: Lock, title: 'Yetki gerekli', tone: 'text-amber-300' },
  not_connected: { icon: WifiOff, title: 'BAĞLI DEĞİL', tone: 'text-amber-300' },
  config: { icon: PlugZap, title: 'YAPILANDIRMA GEREKLİ', tone: 'text-amber-300' },
};
...
export function Modal({ open, onClose, title, children, wide = false, footer }: { open: boolean; onClose: () => void; title: ReactNode; children: ReactNode; wide?: boolean; footer?: ReactNode }) {
  ...
  if (!open) return null;
  return (
    <div className="fixed inset-0 z-[70] flex items-end sm:items-center justify-center bg-ink-950/80 backdrop-blur-sm p-0 sm:p-4" onClick={onClose}>
...
export function Notice({ tone = 'info', children }: { tone?: 'info' | 'warn' | 'error' | 'ok'; children: ReactNode }) {
  const cls = { info: 'bg-sky-500/10 text-sky-200 ring-sky-400/25', warn: 'bg-amber-500/10 text-amber-200 ring-amber-400/25', error: 'bg-rose-500/10 text-rose-200 ring-rose-400/25', ok: 'bg-emerald-500/10 text-emerald-200 ring-emerald-400/25' }[tone];
