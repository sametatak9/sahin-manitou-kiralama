  ...
  return (
    <div className="space-y-4">
      ...
      {q.error ? <ErrorState error={q.error} onRetry={q.reload} /> : q.loading ? <StateView kind="loading" /> : tab === 'inbox' ? (
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-3">
            {q.data.inbox.map((r) => { const done = q.data.converted.has(r.id); return (
              <div key={r.id} className="ops-panel p-4">
                ...
                <div className="flex flex-wrap gap-1 mt-2"><span className="text-[10px] rounded bg-ink-800 px-1.5 py-0.5 text-ink-300">{DEMAND[r.demand] ?? r.demand}</span>{r.budget_range && <span className="text-[10px] rounded bg-ink-800 px-1.5 py-0.5 text-ink-300">{r.budget_range}</span>}{r.ticari_ileti_izni && <span className="text-[10px] rounded bg-emerald-500/15 px-1.5 py-0.5 text-emerald-300">Ticari ileti izni</span>}</div>
            ...
            <table className="w-full text-xs min-w-[760px]">
              ...
              <tbody>{q.data.companies.map((c) => (
                <tr key={c.id} className="border-b border-ink-800/60">
                  <td className="p-3"><div className="font-semibold text-ink-100">{c.firm_name}</div>{c.website && <a href={c.website} target="_blank" rel="noreferrer" className="text-[10px] text-sky-300 inline-flex items-center gap-1"><Globe2 className="w-3 h-3" />{c.website}</a>}</td>
                  <td className="p-3 text-ink-300">{c.ilce ?? '—'}</td><td className="p-3 text-ink-300 max-w-[200px] truncate">{c.need ?? c.project ?? '—'}</td>
                  <td className="p-3">{c.source_url ? <a href={c.source_url} target="_blank" rel="noreferrer" className="text-sky-300 inline-flex items-center gap-1">{c.source}<ExternalLink className="w-3 h-3" /></a> : c.source}</td>
        ...
        q.data.prospects.length === 0 ? <StateView kind="empty" title="Sosyal aday yok" message="Klasik Araçlar → Sosyal Medya bölümündeki aday listeleri burada da görünür." action={<Button variant="ghost" onClick={() => go('tools')}>Klasik araçlar</Button>} /> : (
          <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-2">
            {q.data.prospects.map((p) => (
              <div key={p.id} className="ops-panel !rounded-xl p-3">
                ...
                {p.profile_url && <a href={p.profile_url} target="_blank" rel="noreferrer" className="text-[11px] text-sky-300 underline">Profil</a>}
              </div>))}
          </div>
        )
      )}
