                ...
                <div className="space-y-1.5">{items.map((r) => (
                  <div key={r.id} draggable onDragStart={(e) => e.dataTransfer.setData('text/plain', r.id)} onClick={() => setEditing(r)} className="cursor-grab rounded-lg bg-ink-900 ring-1 ring-ink-700 hover:ring-brand-green/50 p-2.5">
                    ...
                    <div className="flex items-center justify-between mt-1.5 text-[10px]">
                      <span className={cx('font-mono', r.next_action_at && new Date(r.next_action_at) < new Date() ? 'text-rose-300' : 'text-ink-500')}>{r.next_action_at ? `⏱ ${fmtDate(r.next_action_at)}` : relTime(r.created_at)}</span>
                      {r.priority === 'acil' || r.priority === 'yuksek' ? <span className="text-amber-300 font-semibold">{String(r.priority).toUpperCase()}</span> : null}
                    </div>
                  </div>))}</div>
              </div>
            );
