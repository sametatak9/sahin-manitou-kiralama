  ...
  const Card = ({ d, compact = false }: { d: Draft; compact?: boolean }) => {
    ...
    return (
      <div draggable onDragStart={(e) => { e.dataTransfer.setData('text/plain', d.id); setDragging(d.id); }} onDragEnd={() => setDragging(null)}
        ...
        {!compact && pub && <div className="mt-1.5 text-[10px] text-ink-400 truncate">{pub.status === 'published' ? <a onClick={(e) => e.stopPropagation()} href={pub.external_url ?? '#'} target="_blank" rel="noreferrer" className="text-emerald-300 underline">API: {pub.external_post_id}</a> : pub.error}</div>}
      </div>
    );
  };
