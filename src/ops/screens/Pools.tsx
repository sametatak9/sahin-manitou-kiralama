...
export function PoolsScreen() {
  const [tab, setTab] = useState<'banner' | 'video' | 'post'>('banner');
  return (
    <div className="space-y-4">
      ...
      <Tabs value={tab} onChange={setTab} items={[{ id: 'banner', label: 'Banner havuzu' }, { id: 'video', label: 'Video havuzu' }, { id: 'post', label: 'Gönderi metinleri' }]} />
      {tab === 'banner' ? <BannerPool /> : tab === 'post' ? <PostPool /> : <VideoStudioScreen />}
    </div>
  );
}
