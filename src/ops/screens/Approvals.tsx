          ...
          <div className="space-y-4 min-w-0">
              ...
              <Panel title="İçerik" kicker={draft.primary_platform || draft.platform_targets.join(', ')} action={open && <Button variant="subtle" onClick={() => setEdit((v) => !v)} icon={<Pencil className="w-3.5 h-3.5" />}>{edit ? 'Kapat' : 'Düzenle'}</Button>}>
                {edit ? (
                  ...
                  <>
                    ...
                    {draft.hashtags?.length > 0 && <p className="text-xs text-sky-700 mt-2">{draft.hashtags.join(' ')}</p>}
            ...
            {result && (
              <Panel title="Yürütme sonucu" kicker={ap.executed_at ? fmtDateTime(ap.executed_at) : ''}>
                {typeof result.wa_link === 'string' && <a href={result.wa_link} target="_blank" rel="noreferrer" className="inline-flex items-center gap-2 text-sm text-emerald-700 underline"><ExternalLink className="w-4 h-4" /> WhatsApp’ta manuel gönder</a>}
                {typeof result.external_url === 'string' && <a href={result.external_url} target="_blank" rel="noreferrer" className="inline-flex items-center gap-2 text-sm text-emerald-700 underline"><ExternalLink className="w-4 h-4" /> Yayını aç</a>}
                {result.mode === 'manual' && <p className="text-xs text-amber-700 mt-2">{String(result.note ?? 'Manuel işlem gerekli.')}</p>}
