...
import { BookOpen, Cpu, ExternalLink, Link2, Loader2, PlugZap, Radar, RefreshCw, Send, Unplug } from 'lucide-react';
import { callOps, errorText } from '../lib/api';
import { useQuery } from '../lib/hooks';
import { connectionLabel, connectionTone, fmtDateTime, relTime } from '../lib/format';
import type { ConnectorStatus, OpsStatus } from '../lib/types';
  ...
  return (
    <div className="space-y-5">
      <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-3">
        <div>
          <h2 className="font-display text-xl font-semibold text-ink-100">Platform Wall</h2>
          <p className="text-xs text-ink-400">Durumlar gerçektir: CONNECTED yalnızca API’den hesap bilgisi okunduğunda görünür. Resmi API’si olmayan platformlar “MANUEL YAYIN” olarak işaretlenir.</p>
        </div>
      ...
      {data && (
        <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
          ...
          <Panel kicker="OAuth" title="Yönlendirme adresi" action={<Link2 className="w-5 h-5 text-ink-500" />}>
            ...
            <code className="block text-[10px] font-mono text-emerald-300 bg-ink-950 rounded-lg p-2 break-all">{data.redirect_uri}</code>
          </Panel>
        </div>
      )}
        ...
        <Panel key={g.key} kicker={CATEGORY[g.key].kicker} title={CATEGORY[g.key].title}>
          <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-3">
            {g.items.map((c) => {
              ...
              return (
                <div key={c.key} className="rounded-2xl bg-ink-900/60 ring-1 ring-ink-800 p-4 flex flex-col gap-3">
                  <div className="flex items-start gap-3">
                    ...
                    <div className="min-w-0 flex-1">
                      ...
                      <div className="flex flex-wrap gap-1 mt-1">
                        ...
                        {!c.implemented && c.officialApi && <span className="text-[9px] font-mono rounded bg-amber-500/15 px-1.5 py-0.5 text-amber-300">ENTEGRASYON BEKLİYOR</span>}
                      </div>
                    </div>
                  </div>
                  <p className="text-[11px] text-ink-400 leading-relaxed">{c.note}</p>
                  {c.missing_env.length > 0 && <div className="text-[10px] font-mono text-amber-300/90">Eksik secret: {c.missing_env.join(', ')}</div>}
                  {connectedAcc.map((a) => (
                    <div key={a.id} className="flex items-center justify-between gap-2 rounded-lg bg-ink-950 px-2.5 py-1.5 text-[11px]">
                      ...
                      {session.role === 'admin' && <button onClick={() => disconnect(a.id)} className="text-ink-500 hover:text-rose-300" title="Bağlantıyı kes">{busy === a.id ? <Loader2 className="w-3.5 h-3.5 animate-spin" /> : <Unplug className="w-3.5 h-3.5" />}</button>}
                    </div>
                  ))}
