      ...
      {step === 2 && (
        <div className="grid grid-cols-1 xl:grid-cols-[380px_1fr] gap-4">
          ...
          <Panel kicker={`${template.width}×${template.height} · ${FORMATS.find((f) => f.key === brief.format)?.label ?? ''}`} title="Canlı tasarım" action={<Button variant="primary" loading={busy === 'export'} onClick={exportPng} icon={<Download className="w-4 h-4" />}>PNG dışa aktar</Button>}>
            ...
            {exportUrl && <p className="mt-3 text-[11px] text-emerald-300 flex items-center gap-1.5"><Check className="w-3.5 h-3.5" /> Son dışa aktarım: <a className="underline truncate" href={exportUrl} target="_blank" rel="noreferrer">{exportUrl.split('/').pop()}</a></p>}
