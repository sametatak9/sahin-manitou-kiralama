...
import {
  Blocks, Bot, Building2, CalendarRange, CheckCheck, Gauge, Inbox, LogOut, Menu, PlugZap, Radar, Settings, Sparkles, Truck, Wrench, X,
} from 'lucide-react';
...
const GROUPS: Array<{ title: string; items: NavItem[] }> = [
  { title: 'Operasyon', items: [
    { id: 'home', label: 'Operasyon Merkezi', icon: Gauge, hint: 'Bugün ne oluyor' },
    { id: 'approvals', label: 'Onay Akışı', icon: CheckCheck, hint: 'İnsan onayı' },
    { id: 'bots', label: 'Bot Portföyü', icon: Bot, hint: 'Botlar ve görevler' },
  ] },
  { title: 'İçerik', items: [
    { id: 'planner', label: 'İçerik Takvimi', icon: CalendarRange, hint: 'Ay · hafta · gün · kanban' },
    { id: 'studio', label: 'Gönderi Stüdyosu', icon: Sparkles, hint: 'AI + tasarım + önizleme' },
    { id: 'connections', label: 'Platform Duvarı', icon: PlugZap, hint: 'Gerçek bağlantılar' },
  ] },
  { title: 'Müşteri', items: [
    ...
    { id: 'leads', label: 'Lead Gelen Kutusu', icon: Inbox, hint: 'Web + bot adayları' },
  ] },
  { title: 'Sistem', items: [
    { id: 'skills', label: 'Skill & Tool', icon: Blocks, hint: 'Yetenek kütüphanesi' },
    { id: 'settings', label: 'Ayarlar', icon: Settings, hint: 'Marka, AI, ekip, audit' },
    { id: 'tools', label: 'Klasik Araçlar', icon: Wrench, hint: 'Önceki çalışma alanları' },
  ] },
];
const MOBILE: Route[] = ['home', 'planner', 'studio', 'approvals'];
  ...
  const NavLink = ({ item, compact = false }: { item: NavItem; compact?: boolean }) => {
    ...
    return (
      <button onClick={() => { go(item.id); setDrawer(false); }}
        ...
        {badge > 0 && <span className={cx('font-mono text-[10px] font-bold rounded-full px-1.5 py-0.5', item.id === 'approvals' ? 'bg-amber-400 text-ink-950' : 'bg-rose-500 text-white')}>{badge}</span>}
      </button>
    );
  };
  ...
  const sidebar = (
    <div className="flex flex-col h-full">
      ...
      <div className="m-2 rounded-2xl bg-ink-850 ring-1 ring-ink-700 p-3 flex items-center gap-3">
        ...
        <button onClick={onLogout} title="Çıkış" className="p-2 rounded-lg text-ink-400 hover:text-rose-300 hover:bg-ink-800"><LogOut className="w-4 h-4" /></button>
      </div>
    </div>
  );
